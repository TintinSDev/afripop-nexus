...
exports.getMarketStats = async (req, res) => {
  try {
    ...
    // 1. Get unique count of investors from the Share table
    const uniqueInvestors = await prisma.share.groupBy({
      by: ["ownerPhone"],
      _count: true,
    });
    ...
    res.json({
      ...
      investorCount: uniqueInvestors.length, // Real count!
      avgYield: properties.length > 0 ? "9.2%" : "0%", // We can calculate this based on property data later
    });
  } catch (error) {
